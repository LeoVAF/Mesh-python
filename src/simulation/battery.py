...
class Battery:
  ...
  def economic_analysis(self,
    ...
    .. math::
      NPC_{bat} = \text{IC}_{bat} + \text{NPV}_{om} + \text{NPV}_{repl} - \text{NPV}_{sv}.
    ...
    where :math:`T_{repl} = \min\left(I^{\text{lifetime}}_{\text{bat}},\ \dfrac{B^{\text{max}}_{\text{cycles}}}{\sum^{H}_{h=1}B_{\text{cycles}}(h)}\right)` is the time when the equipment must be replaced. The salvage value is calculated as:

    .. math::
      \text{NPV}_{sv} = \frac{\text{IC}_{bat} \cdot \tau_{sv}}{(1 + d)^T}.
