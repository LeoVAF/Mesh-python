...
class Converter:
  ...
  Args:
    reference_cost (:type:`float`): Converter reference cost associated with a rated power of 1 kW in [$].
    cost_exponent (:type:`float`): Exponent cost factor for converter, where a higher power results in a lower cost per kW in [decimal].
    efficiency (:type:`float`): Converter efficiency between 0 and 1.
    lifetime (:type:`float`): Converter lifetime in time intervals.
    resale_rate (:type:`float`): The resale rate of the converter in [decimal].
  '''
  ...
  def __init__(self,
    ...
    ''' Converter reference cost associated with a rated power of 1 kW in [$]. '''
    self.cost_exponent: float
    ''' Exponent cost factor for converter, where a higher power results in a lower cost per kW in [decimal]. '''
    self.efficiency: float
    ''' Converter efficiency between 0 and 1. '''
    self.lifetime: float
    ''' Converter lifetime in time intervals. '''
    self.resale_rate: float
    ''' Resale rate of the converter in [decimal]. '''
  ...
  def economic_analysis(self,
    ...
    - :math:`\text{NPV}_{sv}` is the Net Present Value of the resale value (salvage value) of the converter at the end of its useful life.

    The installation cost is calculated as:

    .. math::
        \text{IC} = C^{ref}_{conv} \cdot \left(\frac{P_{rated}}{1 \ \text{kW}}\right)^{\rho_{conv}}.

    :math:`C^{ref}_{conv}` is the converter reference cost associated with a rated power of 1 kW, :math:`P_{rated}` is the rated power of the distributed energy resources and :math:`\rho_{conv}` is the converter cost-scaling exponent. The operation and maintenance costs are calculated as:
    ...
    Args:
        rated_power (:type:`float`): The power supported by the converter in [kW].
