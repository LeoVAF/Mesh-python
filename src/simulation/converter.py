...
class Converter():
  ...
  def economic_analysis(self,
    ...
    where :math:`T_{repl} = I^{\text{lifetime}}_{\text{conv}}` is the time when the equipament must to be replaced. The salvage value is calculated as:

    .. math::
      \text{NPV}_{sv} = \frac{\text{IC}_{conv} \cdot \tau_{sv}}{(1 + d)^T}.
