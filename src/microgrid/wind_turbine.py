...
class WindTurbine:
  ...
  Args:
    ...
    lifetime (:type:`int | float`): The wind turbine lifetime in years.
  ...
  def __init__(self,
    ...
    ''' The wind turbines lifetime in years. '''
  ...
  def generate_power(self, wind_speed: np.ndarray[np.float64], height_reference: int | float) -> np.ndarray[np.float64]:
    ...
    Args:
      wind_speed (np.ndarray[np.float64]): A numpy array with the wind speed measurements at the reference height.
      height_reference (int | float): The height where the wind speed was measured in [m].
