...
class Inverter():
  ...
  Args:
    cost_per_kw (:type:`int | float`): Inverter cost per kW of nominal capacity in [$/kW].
    cost_scale (:type:`int | float`): Cost scale factor for inverter, where a higher power results in a lower cost per kW in [decimal].
    efficiency (:type:`int | float`): Inverter efficiency between 0 and 1.
    lifetime (:type:`int | float`): Inverter lifetime in time intervals.
  ...
  def __init__(self,
    ...
    ''' Inverter cost per kW of nominal capacity in [$/kW]. '''
    ...
    ''' Total costs of the inverter in the microgrid during the operation simulation in [$]. '''
  ...
  def economic_analysis(self,
    ...
    .. math::
        \text{NPV}_{om} = \sum^{T}_{t=1}\frac{\text{IC}_{inv} \cdot \tau_{om}}{(1 + d)^t}.
